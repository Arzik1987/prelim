...
def experiment_rules(splitn, dname, dsize):                                                                              
    ...
    bicv = BI(depth = par_vals[np.argmax(tmp)])
    bicv.fit(X, y)
    end = time.time()
    sctrain = bicv.score(X, y)
    sctest = bicv.score(Xtest, ytest)     
    fileres.write("\nbicv,na,na,%s,nan,%s,nan,%s" % (sctrain, sctest, (end-start))) 
    fileres.close()
    ...
    for i, j in product([gengmmbic, genkde, genmunge, genrandu, genrandn, gendummy,\
        ...
        sctest = bicv.score(Xtest, ytest)                                       
        fileres.write("\nbicv,%s,%s,%s,%s,%s,nan,%s" % (i.my_name(), j.my_name(), sctrain, scnew, sctest, (end-start)))
