...
class Meta_xgb_bal:
    ...
    def fit(self, X, y):
        ...
        spv = (y.shape[0] - nz)/nz
        tmp = RandomizedSearchCV(XGBClassifier(nthread=1, verbosity=0, use_label_encoder=False, scale_pos_weight=spv), self.params_,
                                 random_state=self.seed_, cv=self.cv_, n_iter=50, n_jobs=1, scoring = 'balanced_accuracy')
        tmp.fit(X, y)
