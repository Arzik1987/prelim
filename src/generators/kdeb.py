...
class Gen_kdeb:
    ...
    def fit(self, X, y = None):
        ...
        if self.knn_ == 0:
            ...
            sys.exit("The dataset is too small or the knn value is too large")
        else:
            self.dist_ = np.mean(NearestNeighbors(n_neighbors = self.knn_).fit(X).kneighbors()[0][:, self.knn_ - 1])
        return self

    def sample(self, n_samples): # http://extremelearning.com.au/how-to-generate-uniformly-random-points-on-n-spheres-and-n-balls/
        d = self.X_.shape[1]
