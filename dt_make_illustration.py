...
def get_result(fname):
    ...
    tmp.columns = ['alg', 'gen', 'met', 'tra', 'new', 'tes', 'nle','tme', 'nge', 'fid1', 'fid2']
    tmp = tmp[(tmp['alg'] == 'dt')]
...
a = res[res['dat'] == 'turbine']
a = a[~(a['gen'] == 'na')]
a[["fid1", "fid2"]] = a[["fid1", "fid2"]].apply(pd.to_numeric)
a = a[['alg', 'gen', 'met', 'nge', 'tes', 'nle', 'tme', 'fid1', 'fid2']].groupby(['alg', 'gen', 'met', 'nge']).mean()
a.to_csv(WHERE + 'a.csv')
a = pd.read_csv(WHERE + 'a.csv', delimiter = ",")
os.remove(WHERE + "a.csv")
a.pivot('nge', 'gen', 'fid1').sort_values('nge')
